# Substrate Runtime Toolbox: srtool v0.18.5
...
-   `paritytech/srtool:1.98.1-0.18.5`

-   `paritytech/srtool:1.98.1`

The tags not mentioning the build version always point to the latest one. In the example above, `paritytech/srtool:1.98.1` is the same image than `paritytech/srtool:1.98.1-0.18.5`.
...
The last version hosted on Gitlab has been built using Rust Stable 1.98.1. It is tagged as v0.18.5 and there is no plan on updating the Gitlab repository further. New versions will be available from [this repository](https://www.github.com/paritytech/srtool) only. The functionalities remain the same so you can (and should!) simply swap `chevdor/srtool` for `paritytech/srtool` in your workflows. The [srtool-actions](https://github.com/chevdor/srtool-actions) will remain available as `chevdor/srtool-actions@<version>` and will be updated to point at the paritytech image.
...
`srtool` includes a command that helps finding runtimes in a repo.
        ...
        podman run --rm -it \
            -v $REPO:/build \
            `paritytech/srtool:1.98.1-0.18.5` scan
