# Substrate Runtime Toolbox: srtool v0.18.4
...
-   `paritytech/srtool:1.98.1-0.18.4`

-   `paritytech/srtool:1.98.1`

The tags not mentioning the build version always point to the latest one. In the example above, `paritytech/srtool:1.98.1` is the same image than `paritytech/srtool:1.98.1-0.18.4`.
...
The last version hosted on Gitlab has been built using Rust Stable 1.98.1. It is tagged as v0.18.4 and there is no plan on updating the Gitlab repository further. New versions will be available from [this repository](https://www.github.com/paritytech/srtool) only. The functionalities remain the same so you can (and should!) simply swap `chevdor/srtool` for `paritytech/srtool` in your workflows. The [srtool-actions](https://github.com/chevdor/srtool-actions) will remain available as `chevdor/srtool-actions@<version>` and will be updated to point at the paritytech image.
...
Creating an alias helps hiding the docker complexity behind one simple command. We will see more powerful options but this one is simple enough.
        ...
        export RUSTC_VERSION=1.98.1; export PACKAGE=kusama-runtime; alias srtool='docker run --rm -it -e PACKAGE=$PACKAGE -v $PWD:/build -v $TMPDIR/cargo:/cargo-home paritytech/srtool:$RUSTC_VERSION'
...
will output something that looks like this:
        ...
        🧰 Substrate Runtime Toolbox - srtool v0.18.4 🧰
                  - by Chevdor -
        🏗  Building polkadot-runtime as release using rustc 1.98.1
...
The `info` and `version` scripts pass any arguments you pass to the script to `jq`. So you can play with `c` (compact), `-M` (monochrome), `-C` color output. For instance `docker run --rm -it -v $PWD:/build chevdor/srtool:1.98.1 info -cM` shows a monochrome output on a single line.
...
`srtool` includes a command that helps finding runtimes in a repo.
        ...
        podman run --rm -it \
            -v $REPO:/build \
            `paritytech/srtool:1.98.1-0.18.4` scan
...
So say you want to build a builder for rustc 1.98.1:

        RUSTC_VERSION=1.98.1 && docker build --build-arg RUSTC_VERSION=$RUSTC_VERSION -t paritytech/srtool:$RUSTC_VERSION .
